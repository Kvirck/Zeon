...
const Collaps = ({ question, toggle, text, i, showBtn }) => {

    return (
        <div className={style.collaps}>
            <div className={style.collaps__text}>
                <p className={style.collaps__title}>{question}</p>
                <button className={showBtn === i  ? style.rotate : style.noRotate} onClick={() => toggle(i)}>
            ...
            <p className={showBtn === i ? `${style.collaps__description} ${style.active}` : style.collaps__description}> {text} </p>
        </div>
    )
}
