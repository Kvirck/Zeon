...
const ProductsPage = () => {
    ...
    return (
        <div className="container">
            ...
            {data.map(item => {
                return (
                    <div key={item.id} className={style.productsPage}>
                        ...
                        <div className={style.productsPage__text}>
                            <p className={style.productsPage__title}>{item.colors[0].name}</p>
                            <p className={style.productsPage__identification}>Артикул: <span>{item.colors[0].articul}</span></p>
                            <div className={style.productsPage__color}>
                                <p >Цвет: </p>
                                {item.colors_name.map((color, i) =>
                                    <div key={color[0] + i}>
                            ...
                            <div className={style.productsPage__price}>
                                <p className={style.productsPage__priceNew}>
                                    {console.log(item.colors[0].current_price)}
                                    {item.colors[0].current_price} {item.colors[0].current_price ? ' р ' : ''}
                                    <span className={item.colors[0].current_price ? style.productsPage__priceOld : style.productsPage__priceNew}>{item.colors[0].price} р</span>
                                </p>
                            </div>
                            <p className={style.productsPage__info}>О товаре:</p>
                            <p className={style.productsPage__description}>{item.colors[0].description}</p>
                            <div className={style.productsPage__characteristics}>
                                <div className={style.productsPage__characteristicsLeft}>
                                    <p className={style.productsPage__characteristics__data}>Размерный ряд:<span>{item.colors[0].size}</span></p>
                                ...
                                <div className={style.productsPage__characteristicsRight}>
                                    <p className={style.productsPage__characteristics__data}>Состав ткани:<span>{item.colors[0].material} </span></p>
                                    <p className={style.productsPage__characteristics__data}>Материал:<span>{item.colors[0].property} </span></p>
                                </div>
                            </div>
