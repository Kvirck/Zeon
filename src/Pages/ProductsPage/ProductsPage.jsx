...
const ProductsPage = () => {
    ...
    const [show, setShow] = useState(Basket.products.find((prod) => prod.id === post.id  &&  prod.colors.id === post.colors.id ))
    ...
    const handleColorIndex = (id) => {
        ...
        setIndex(i);
        console.log(`color = ${post.colors[index].articul}`);
            ...
            {post && (
                <div className={style.productsPage}>
                    ...
                    <div className={style.productsPage__text}>
                        <p className={style.productsPage__title}>{post.colors[index].name}</p>
                        <p className={style.productsPage__identification}>Артикул: <span>{post.colors[index].articul}</span></p>
                        ...
                        <p className={style.productsPage__description}>{post.colors[index].description}</p>
                        <div className={style.productsPage__characteristics}>
                            <div className={style.productsPage__characteristicsLeft}>
                                <p className={style.productsPage__characteristics__data}>Размерный ряд:<span>{post.colors[index].size}</span></p>
                            ...
                            <div className={style.productsPage__characteristicsRight}>
                                <p className={style.productsPage__characteristics__data}>Состав ткани:<span>{post.colors[index].material} </span></p>
                                <p className={style.productsPage__characteristics__data}>Материал:<span>{post.colors[index].property} </span></p>
                            </div>
                        </div>
                        <div className={style.productsPage__btn}>
                            <AddBasket setShow={setShow} show={show} post={post} />
